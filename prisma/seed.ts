...
async function main() {
  ...
  const kopi = await prisma.category.upsert({
    where: { name: 'Kopi' },
    update: {},
    create: { name: 'Kopi', icon: 'Coffee', color: '#d97706' },
  })
  const makanan = await prisma.category.upsert({
    where: { name: 'Makanan' },
    update: {},
    create: { name: 'Makanan', icon: 'UtensilsCrossed', color: '#059669' },
  })
  const minuman = await prisma.category.upsert({
    where: { name: 'Minuman Lain' },
    update: {},
    create: { name: 'Minuman Lain', icon: 'GlassWater', color: '#2563eb' },
  })
  await prisma.category.upsert({
    where: { name: 'Snack' },
    update: {},
    create: { name: 'Snack', icon: 'Cookie', color: '#7c3aed' },
  })
  const bahan = await prisma.category.upsert({
    where: { name: 'Bahan Baku' },
    update: {},
    create: { name: 'Bahan Baku', icon: 'Package', color: '#ea580c' },
  })
  console.log('✅ Categories created')

  // --- Data aktual dari Excel ---
  const bahanRows = [
  ...
  const menuPriceMap = new Map(
    hargaRows.map((h) => [
      h.namaMenu,
      h.hargaSatuan ?? h.hargaHot ?? h.hargaIce ?? 0,
    ])
  )

  // --- Products (aktual, tanpa data random) ---
  const products = [
  ...
  // --- Tabel baru: bahan, logistik, harga ---
  ...
  // --- Transactions (aktual pembelian dari logistik + bahan, tanpa random) ---
  ...
  console.log(`✅ ${transactionData.length} Transactions created (aktual)`)
