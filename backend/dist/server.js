...
const toSingle = (value) => (typeof value === 'string' ? value : undefined);
...
app.get('/health', (_req, res) => ok(res, { service: 'warkop-backend', status: 'healthy' }, 'Backend healthy'));
app.post('/api/auth/login', async (req, res) => {
    try {
        const parsed = loginSchema.safeParse(req.body);
        if (!parsed.success)
            return fail(res, 'Payload login tidak valid', 422, parsed.error.flatten());
        ...
        if (!valid)
            return fail(res, 'Username atau password salah', 401);
        return ok(res, { id: user.id, username: user.username, name: user.name, role: user.role }, 'Login berhasil');
    }
    catch (error) {
...
app.post('/api/products', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Payload produk tidak valid', 422, parsed.error.flatten());
...
app.patch('/api/products/:id', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Payload update produk tidak valid', 422, parsed.error.flatten());
...
app.post('/api/transactions', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Payload transaksi tidak valid', 422, parsed.error.flatten());
...
app.post('/api/logistic-items', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Payload logistic item tidak valid', 422, parsed.error.flatten());
...
app.get('/api/dashboard', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Query dashboard tidak valid', 422, parsed.error.flatten());
        ...
        const menuProducts = menuCategory === 'all' ? allProducts : allProducts.filter((p) => p.category.name === menuCategory);
        const categoryProductNames = new Set(menuProducts.map((p) => p.name.toLowerCase()));
        const getProductName = (description) => description.replace(/\s*x\d+$/i, '').trim().toLowerCase();
        const includeIncome = (description) => menuCategory === 'all' || categoryProductNames.has(getProductName(description));
        const filteredTransactions = transactions.filter((t) => {
            if (txType !== 'all' && t.type !== txType)
                return false;
            if (method !== 'all' && t.method !== method)
                return false;
            if (t.type === 'income' && !includeIncome(t.description))
                return false;
            return true;
        });
        ...
        const sumByType = (rows, type) => rows.filter((t) => t.type === type).reduce((sum, t) => sum + t.amount, 0);
        const countIncome = (rows) => rows.filter((t) => t.type === 'income').length;
        ...
        const topProducts = [...topMap.entries()]
            ...
            .slice(0, 5);
        const potentialRevenueByCategory = {
            Kopi: 0,
            Makanan: 0,
            'Minuman Lain': 0,
        };
        for (const product of allProducts) {
            if (!['Kopi', 'Makanan', 'Minuman Lain'].includes(product.category.name))
                continue;
...
app.get('/api/reports', async (req, res) => {
    try {
        ...
        if (!parsed.success)
            return fail(res, 'Query reports tidak valid', 422, parsed.error.flatten());
        ...
        const products = menuCategory === 'all' ? allProducts : allProducts.filter((p) => p.category.name === menuCategory);
        const productNames = new Set(products.map((p) => p.name.toLowerCase()));
        const normalize = (description) => description.replace(/\s*x\d+$/i, '').trim().toLowerCase();
        const includeIncome = (description) => menuCategory === 'all' || productNames.has(normalize(description));
        ...
        transactions
            .filter((t) => t.type === 'income' && includeIncome(t.description))
            .forEach((t) => {
            if (!methodData[t.method])
                methodData[t.method] = 0;
            methodData[t.method] += t.amount;
        });
        const lowStockProducts = products.filter((p) => p.minStock > 0 && p.stock <= p.minStock);
        const transactionCount = transactions.filter((t) => t.type !== 'income' || includeIncome(t.description)).length;
        return ok(res, {
            totalIncome,
            totalExpense,
